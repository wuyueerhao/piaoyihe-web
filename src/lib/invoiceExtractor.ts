  ...
  try {
    ...
    const cleanCompanyName = (name: string) => {
      ...
      while (cleaned !== lastCleaned) {
        lastCleaned = cleaned;
        cleaned = cleaned.replace(/^\\d{8,}/, ''); // 移除开头的长串纯数字(如发票号码)
        cleaned = cleaned.replace(/^(?=[0-9A-Z]*[0-9])[0-9A-Z]{15,20}/i, ''); // 移除开头的税号
        cleaned = cleaned.replace(/^\\d{4}[年/.\\-][\\d]{1,2}[月/.\\-][\\d]{1,2}日?/, ''); // 移除开头的完整日期
        cleaned = cleaned.replace(/^\\d{1,2}月\\d{1,2}日/, ''); // 移除开头的短日期
        cleaned = cleaned.replace(/^(?:统一社会信用代码|纳税人识别号|密码区|开票日期|发票号码|机器编号|校验码|收款人|复核人|开票人|购买方信息|销售方信息|购买方|销售方|项目名称|规格型号|单位|数量|单价|金额|税率|税额)[：:]?/g, ''); // 移除错位的表头
      }
      return cleaned;
    };
